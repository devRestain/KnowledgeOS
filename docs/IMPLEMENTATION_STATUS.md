...
- Preserve the product boundary: core capability sessions `C01` through `C19` are complete and `C20` is the next planned core slice; the interactive MacBook completion target is `C23` and `C24` is optional core reliability.
