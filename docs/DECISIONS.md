...
- Keep the E03 LaunchAgent provider-free and periodic; use local Ollama as the only current provider lane, defer remote model routes, and keep provider promotion, local activation, plugin settings, device actions, and Git network effects behind their own gates. The authorized E05 slice exercised one explicit local route without changing the E03 boundary.
...
- Keep plugin configuration, GUI execution, and device behavior as separately authorized evidence; serialized manifests and settings never prove that a plugin or GUI workflow ran successfully. The E05 device result is recorded separately from the P01 profile audit.
