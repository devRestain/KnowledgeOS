...
- Treat the F01-F06 GUI-first period-note and `vaultctl` boundary as a registered target contract, not as current implementation evidence; do not claim source or Blueprint conformance until each slice passes its acceptance checks.
